...
- Day 5: Trained and evaluated the visual helper with `scripts/train_graph_model.py` / `scripts/eval_graph_model.py`, saving adapters in `models/adapters/graph_reader_v1/` and metrics in `artifacts/results/graph_reader_v1/`.
...
- Day 14: Trained adapter_v2, benchmarked it against adapter_v1 (slight JSON/schema gains), and packaged every artefact into `artifacts/handoff/day14_artifacts/` for the post-automation review team.
...
- saved the trained weights as `models/adapters/graph_reader_v1` and kept a note of the run in `artifacts/logs/train/graph_reader_v1` so we remember what happened.
- wrote the score sheet to `artifacts/results/graph_reader_v1/metrics.json` and the eight example write-ups to `artifacts/results/graph_reader_v1/samples.jsonl`; two of those examples are still a bit too different from the human wording.
- anything in `models/` is a saved model we can load later, and anything in `artifacts/results/` is a score report from the most recent test run.
...
- First 100-model batch (`data/parsed/auto_drafts.jsonl`, log in `artifacts/logs/auto_generate/run.jsonl`) still gave wobbly physics and bad numerics, so we rejected them all via `scripts/review_seed.py`.
...
- Ran the filter over `data/parsed/seed_train.jsonl`, keeping 459 good MCQs and logging the single broken item plus 106 “difficulty missing” warnings to `data/review/day9_seed_train_flagged.jsonl` and `artifacts/reports/day9_seed_train.json`.
- Pointed the same tool at the 20 model drafts (`data/parsed/auto_drafts.jsonl`) and confirmed every item now passes the schema checks; coverage lives in `artifacts/reports/day9_auto_drafts.json`.
- Summarised the review log (`data/review/seed_notes.jsonl`) so we can see that all 623 entries still need explicit decisions; the markdown summary sits in `artifacts/reports/day9_review_notes.md`.
...
- Ran a one-step smoke test using `Qwen/Qwen2.5-0.5B-Instruct` (`--device-map cpu`) to prove the pipeline end-to-end; the run logged `loss≈3.43`, saved weights in `models/adapters/adapter_v1/`, and wrote structured events to `artifacts/logs/train/adapter_v1.jsonl`.
...
- Full CPU run over `data/formatted/test.jsonl` took ~15 minutes (0.5B base + adapter). Results land in `artifacts/results/adapter_v1/metrics.json` with per-item dumps in `artifacts/results/adapter_v1/samples.jsonl`.
...
Day 13 recap:
- Added `scripts/prepare_refresh.py` to scan `artifacts/results/adapter_v1/samples.jsonl`, flag the 46 weakest MCQs (JSON errors, duplicate distractors, answer mismatches), and dump their source question parts to `data/parsed/refresh_sources.jsonl` with a Markdown summary for reviewers.
...
- Ran `scripts/filter_balance.py` on the drafts so `data/filtered/refresh_candidates.jsonl` holds 40 ready-to-review MCQs with zero warnings; coverage logged to `artifacts/reports/day13_refresh.{json,md}`.
...
Day 14 recap:
- Trained a second adapter via `scripts/train_lora.py` (`adapter_v2`, CPU one-step sanity pass), logging to `artifacts/logs/train/adapter_v2.jsonl` and appending metadata to `config/adapters.yaml`.
- Evaluated adapter_v2 with `scripts/eval.py` (results in `artifacts/results/adapter_v2/`), then compared both adapters using `scripts/compare_adapters.py`, which shows ~+2% JSON-valid and ~+6% schema-valid but answer_match still at 0%.
- Collected all downstream artefacts (refresh candidates, metrics, summaries, adapters, feedback log) using `scripts/create_handoff_bundle.py`; the bundle plus manifest lives in `artifacts/handoff/day14_artifacts/`.
