...
- Initialise Git LFS and track `data/` + `artifacts/logs/` so large binaries stay out of normal git history; record commands in `docs/internal-notes.md`
...
- Run `scripts/extract_pdf.py` (tune with flags like `--render-dpi`, `--min-raster-contrast`, `--drawing-cluster-gap`) to extract per-page text and targeted image crops; store logs in `artifacts/logs/extract/`.
...
- Review `artifacts/logs/extract/run_*.jsonl` for any `status: "error"` PDFs, adjust parameters if needed, and note issues/fixes in `docs/internal-notes.md`
...
- Create `scripts/analyse_graphs.py` to detect chart-like images, merge related vector clips emitted by `scripts/extract_pdf.py` into per-graph composites, and harvest rich geometry features (axis detection, line statistics, keypoints, component counts) into `data/graph_analysis/` while logging to `artifacts/logs/graph_analysis/`.
...
- Run `scripts/analyse_graphs.py` to populate geometry metadata; run `scripts/describe_graphs.py` afterwards so every graph has both structured features and readable text (logs go to `artifacts/logs/graph_descriptions/`).
- While OCR/graph passes run, start cleaning noisy text (headers, watermarks) and log fixes in `artifacts/logs/cleanup/`.
...
- Build `scripts/train_graph_model.py`: it streams each PNG directly from disk, pairs it with the human summary, and fine-tunes `Qwen/Qwen2.5-VL-7B-Instruct` in 4-bit (QLoRA) with LoRA rank 16 / α 32 / dropout 0.05, gradient accumulation 4 (effective batch size 4). Training logs land in `artifacts/logs/train/graph_reader_v1/run_*.json` and the adapter saves to `models/adapters/graph_reader_v1/`.
- Add `scripts/eval_graph_model.py`: reload the base model + adapter, generate summaries for the 8 held-out diagrams, and measure Rouge-L, BLEU-1, and sentence-transformer cosine similarity (flag anything <0.70). Metrics write to `artifacts/results/graph_reader_v1/metrics.json` and `samples.jsonl` for easy review.
...
- Built the SQLite lookup (`scripts/build_question_index.py --force`) so `data/index/questions.db` now holds questions (1004), parts (1126) and assets (3194) with run logs in `artifacts/logs/index/`.
...
- Generated 100 auto drafts (`data/parsed/auto_drafts.jsonl`) plus run log (`artifacts/logs/auto_generate/run.jsonl`). Common issues captured: poor numeric accuracy (e.g. mass mis-scaling), weak particle-physics reasoning, AO/topic left null.
...
- Run the filter over `data/parsed/seed_train.jsonl` (459 kept, 1 dropped) and the fresh `data/parsed/auto_drafts.jsonl`, saving markdown + JSON reports under `artifacts/reports/day9_*.{md,json}` and cleaned copies in `data/filtered/`.
...
- Generate train/val/test splits (80/10/10) automatically, writing stats to `artifacts/results/dataset_stats.json` and recording snapshot hashes in `docs/internal-notes.md`.
...
- Stream training metrics to `artifacts/logs/train/adapter_v1.jsonl` and store checkpoints in `models/adapters/adapter_v1/`, updating `config/adapters.yaml` automatically.
...
- ✅ `python scripts/prepare_refresh.py --include-answer-mismatch` flagged 46 weak outputs and wrote `data/parsed/refresh_sources.jsonl` plus a summary (`artifacts/results/adapter_v1/refresh_summary.{json,md}`).
- ✅ `python scripts/auto_generate.py --input data/parsed/refresh_sources.jsonl --output data/parsed/refresh_drafts.jsonl --limit 60 --allow-existing` produced 40 clean drafts (one stubborn part still fails due to sci-notation placeholders) which were filtered into `data/filtered/refresh_candidates.jsonl` with reports under `artifacts/reports/day13_refresh.{json,md}`.
...
- Use the new variant reviewer (CLI or `streamlit run ui/variant_review.py`) to sample five random question parts, compare their five variants, pick a favourite, and mark each variant `accept`/`reject`; decisions land in `data/review/variant_choices.jsonl`.
