...
def collect_decision() -> Optional[Dict[str, object]]:
    try:
        command = input("Command [a=accept, r=reject, s=skip, b=back, q=quit] > ").strip()
    except EOFError:
        return {"action": "quit"}
    if not command:
        return None
    ...
    if action not in {"a", "r", "s"}:
        print("Unrecognised command; enter a/r/s/b/q optionally followed by a comment.")
        return None
    return {
